print(scenario) #must be specified in script loading model_setup

n.active <- 1 #1 set of active states (each set includes subclinical and symptomatic)
n.ltbi <- 2 
n.ages <- age_groups #2 age groups
n.types <- 2 #resistors and susceptibles - not using (resistor pops set to 0)

## dummy array with indices
matind <- array(1:((2 + n.active*2 + n.ltbi + 1)*n.types*n.ages),c((2 + n.active*2 + n.ltbi + 1),n.types,n.ages))
states <- c('U', paste0('L', 1:n.ltbi), paste0('AA', 1:n.active) , paste0('SA', 1:n.active), 'R', 'C')
dimnames(matind)[[1]] <- states
...
#clearance rate - if using
if(scenario=="progression") {
  c.2 <- 1/(10 - 1/0.5)
  c <- rep(c.2, n.ltbi) #10 yrs on average before clearance, subtract off first 2 years when in early LTBI
} else {
  c.2 <- 0
  c <- rep(c.2, n.ltbi)
}

# reactivation rate - by age and LTBI type (columns = ages, rows= different LTBI states, if there are > 2 LTBI states)
...
phi <- array(data=0, dim=c(n.ltbi-1, n.ages))
phi[,] <- phi_full[1:(n.ltbi-1),]

# progression to care-seeking
r1 <- 2.05

# regression to pre-care-seeking (fixed at 0)
r2 <- 0
...
ystart <- array(1:((2 + n.active*2 + n.ltbi + 1)*n.types*n.ages),c((2 + n.active*2 + n.ltbi + 1),n.types,n.ages))


ystart.epi <- matrix(c(c(0.3, 0.1, rep(0.5/(n.ltbi-1), n.ltbi-1), rep(0.002/(n.active*2), n.active*2), 0.1-0.002, 0),
                       c((rel.inf.type[1]) + 0.3*rel.inf.type[2], 
                         rel.inf.type[2]*c(0.1,  rep(0.5/(n.ltbi-1), n.ltbi-1), 
                                                  rep(0.002/(n.active*2), n.active*2), 0.1-0.002, 0))), 
                     nrow=n.types, byrow=T)
ystart.type <- c(prop.susceptible,prop.resistant)
ystart.age <- c(0.37,0.63) 
			
for(je in 1:(2 + n.active*2 + n.ltbi + 1)){
	for(js in 1:n.types){
		for(ja in 1:n.ages){
